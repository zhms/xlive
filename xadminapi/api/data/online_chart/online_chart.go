...
func Init() {
	xglobal.ApiV1.POST("/get_online_data", admin.Auth("数据分析", "峰值图表", "查", ""), get_online_data)
}

type get_online_data_req struct {
	StartTime string `json:"start_time" validate:"required"` // 开始时间
	EndTime   string `json:"end_time" validate:"required"`   // 结束时间
}

type get_online_data_res struct {
	Total int64               `json:"total"` // 总数
	Data  []*model.XStatistic `json:"data"`  // 数据
}

// @Router /get_online_data [post]
// @Tags a
// @Summary b
// @Param x-token header string true "token"
// @Param body body get_online_data_req true "请求参数"
// @Success 200  {object} get_online_data_res "响应数据"
func get_online_data(ctx *gin.Context) {
	var reqdata get_online_data_req
	...
	response := new(get_online_data_res)
	...
	itb = itb.Where(tb.RecordType.Eq("mx"))
	{
